...
class KlientsAddressView extends AbstractMigration{
	...
	public function change()
	{
		$this->execute(
    		...
    		VIEW `klient_address` AS (SELECT
				...
				kd.`firstname`, kd.`lastname`, kd.`kind`, kd.`firm_name`, kd.`organ`, kd.`ico`,
				 `a`.`city`, `a`.`street`, `a`.`cp`, `a`.`co`, `a`.`byt_cis`, `a`.`patro`
